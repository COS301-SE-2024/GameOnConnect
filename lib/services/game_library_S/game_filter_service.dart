...
class GameFilterService {
  static Future<FilterList> createFilterList() async {
    ...
    final storeFilters =
        await _fetchFilters('https://api.rawg.io/api/stores?key=$apikey');
    final genreFilters =
        await _fetchFilters('https://api.rawg.io/api/genres?key=$apikey');
    final tagFilters =
        await _fetchFilters('https://api.rawg.io/api/tags?key=$apikey');
    final platformFilters =
        await _fetchFilters('https://api.rawg.io/api/platforms?key=$apikey');
